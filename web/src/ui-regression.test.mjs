...
assert.match(app, /if \(sameNamespace\) \{[\s\S]*?removedSessionIDsRef\.current\.add\(sessionID\)[\s\S]*?setSessions\(\(current\) => current\.filter/, 'a successful delete must tombstone and remove its row even after same-namespace navigation')
assert.match(app, /replaceMutationContext\(null\)[\s\S]*?removedSessionIDsRef\.current\.add\(sessionID\)/, 'deleting the selected session must restore its tombstone after context replacement')
